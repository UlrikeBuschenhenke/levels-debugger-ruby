...
export default class Debugger {
  ...
  //Called if a step over in the currently active replay is made.
  ...
  replayRunToNextBreakpoint() {
    if (this.recorder.endOfTape()) {
      ...
      if (this.breakpointManager.isBreakpoint){
        this.setState(this.recorder.replayStep());
        this.emitReplayState(this.getPosition());
      } else {
  ...
  //Called if a run to end of method in the currently active replay is made.
  replayRunToEndOfMethod() {
    if (this.recorder.endOfTape()) {
      this.emitEndOfReplayTape();
    } else {
      if (!this.isCallStackEmpty()) {
        const callId = this.getCallStack[0].getCallId();
        const state = this.recorder.replayStateforCall();
        this.setState(this.recorder.replayStep());
        this.emitReplayState(this.getPosition());
      }
    }
  ...
  popFromCallStack() {
    ...
    if (!this.isCallStackEmpty()){
      if (!this.isReplay) {
        const callId = callStack[0].getCallId();
        this.recorder.recordLeavePoint(callId);
      }
