...
export default class DebuggerPresenter {
  ...
  updatePosition(message) {
    ...
    this.emitPositionUpdated(message);
  }
  ...
  pushOntoCallstack(message) {
    const msg = new PushOntoCallStackMessage(message);
    this.debugger.pushOntoCallStack(callStackEntryFromString(msg, this.callId));
  ...
  step() {
    if (this.areSteppingCommandsEnabled()) {
      ...
      //Bisher keine Funktionalität enthalten. Notwendig?
      //this.debugger.step();
      this.sendStepSignal();
    }
  }
